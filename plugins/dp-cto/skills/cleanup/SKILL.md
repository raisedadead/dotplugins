---
name: cleanup
description: "Sprint-boundary cleanup. Clears local cache, closes eligible epics, cleans dp-cto labels from completed work, handles sprint closure, prunes ralph states and orphaned worktrees. Deliberate action — invoke when a body of work is done."
---
...
2. **Ralph state files** — `.claude/ralph/*.md` with `status: complete`, `status: exhausted`, or `status: cancelled` in frontmatter (skip `status: running` — that's an active loop)
...
Present the summary to the user via `AskUserQuestion` (confirm): **"Cleanup will clean: cache.json, N ralph states, N closeable epics, N stale dp-cto labels[, active sprint 'NAME'][, N orphaned worktrees]. Proceed?"**
