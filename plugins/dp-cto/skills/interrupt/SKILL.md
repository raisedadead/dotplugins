...
# dp-cto:interrupt — Context-Preserving Work Suspension
...
**"Epic `{epic-id}` suspended ({closed}/{total} tasks complete). Start new work with `/dp-cto:start` or return with `/dp-cto:resume`."**
