...
# dp-cto:sprint — Sprint Lifecycle Management
...
| "I'll implement fixes from the retro"        | You are CTO. Delegate fixes via /dp-cto:start.                 |
...
If `bd` is not on `$PATH`, report: **"bd CLI not found. Install beads to use /dp-cto:sprint."** and STOP.
...
If no `.beads/` directory, report: **"No beads database found. Run /dp-cto:start to create a plan first."** and STOP.
...
If an active sprint already exists, report: **"Sprint '{title}' is already active. Close it with /dp-cto:sprint (Close) before starting a new one."** and STOP.
...
If no feature epics found, report: **"No feature epics in backlog. Create features with /dp-cto:start first, then add them to the sprint."** Remove the sprint:planning label, close the sprint epic, and STOP.
...
**"Sprint {N}: {THEME} created (epic `{sprint-id}`) with {count} features. Use /dp-cto:sprint (Review) to track progress."**
...
If no active sprint found, report: **"No active sprint. Create one with /dp-cto:sprint (Plan)."** and STOP.
...
3. Track:
   ...
   - Tasks that required multiple iterations (re-dispatched or sent to ralph)
...
- Multiple iterations or ralph invocations
...
If the user declines, report: **"Sprint remains active. Complete remaining features or re-run /dp-cto:sprint (Close) when ready."** and STOP.
