---
name: work-stop-loop
description: "Cancel an active dp-cto:work-run-loop loop. Lists active work-run-loop state files and lets you choose which to cancel."
---

# Cancel dp-cto:work-run-loop Loop
...
- If no files found: say "No active work-run-loop sessions found." and STOP.
...
- If no files have `status: running`: say "No active work-run-loop sessions found." and STOP.
...
2. Report: "Cancelled work-run-loop session {SESSION_ID} at iteration {N}. State file preserved at .claude/ralph/{SESSION_ID}.md"
