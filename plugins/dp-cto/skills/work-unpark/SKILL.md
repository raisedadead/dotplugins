---
name: work-unpark
description: "Restore a previously suspended epic. Queries beads for suspended epics, restores context from suspension notes, and shows actionable next steps. Use after /dp-cto:work-park to return to prior work."
---

# dp-cto:work-unpark — Restore Suspended Work

## Stage Enforcement

This skill is allowed from `idle` only. The PreToolUse hook enforces this. You must not have an active epic — if one exists, use `/dp-cto:work-park` to suspend it first, then resume the target epic.
...
- If the query returns empty, null, or an empty array: say **"No suspended epics found. Use `/dp-cto:work-plan` to begin new work."** and STOP.
...
- If stage is `executing` or `planned`: **"Continue with `/dp-cto:work-run` to dispatch ready tasks."**
- If stage is `polishing`: **"Continue with `/dp-cto:work-polish` to complete the review phase."**
